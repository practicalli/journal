...
[Material for MkDocs](#practicalli) is the static site generator tool used to create all the Practical.li websites.  This project is now in maintenance mode and a project called Zensical is under active development. It seems that there should be a fairly smooth migration of existing projects to the new tool :crossed_fingers:
...
The maintainers of Material for MkDocs are now developing [Zensical](https://zensical.org/docs/get-started/){target=_blank}, a complete re-write of the static site generator.
...
!!! NOTE "Remove remote computer from known_hosts by IP address"
    ```shell
    ssh-keygen -f '/home/practicalli/.ssh/known_hosts' -R 'rogueone'
    ```

??? EXAMPLE "Output: Removing remote computer by IP address"
    ```shell-output
    ❯ ssh-keygen -f '/home/practicalli/.ssh/known_hosts' -R 'rogueone'
    # Host rogueone found: line 1
    # Host rogueone found: line 2
    # Host rogueone found: line 3
    /home/practicalli/.ssh/known_hosts updated.
    Original contents retained as /home/practicalli/.ssh/known_hosts.old
    ```

!!! NOTE "Secure Shell to remote computer"
    ```shell
    ssh practicalli@rogueone
    ```
...
??? EXAMPLE "Output: Removing remote computer by IP address"
    ```shell-output
    ❯ ssh practicalli@rogueone
    Warning: Permanently added 'rogueone' (ED25519) to the list of known hosts.
    practicalli@rogueone:~$ Shared connection to rogueone closed.
    ```
