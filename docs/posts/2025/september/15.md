...
I continue to use a [Hyprland & Wayland](#hyprland) based system for my main computer and it is working well.

The huge push for 'AI' services over the last year has been mostly hype, so I have dismissed much of it. I have [started to consider how I gather and organise information](#ai-in-reality) though, to see if there are ways that could be optimised.

Downsize my life continues with my older bicycles and accessories being recycled or upcycled.  I am also investigating where are [effective places to sell my bicycle](https://mynextbike.co.uk/blogs/selling/where-to-sell-your-used-bike){target=_blank}.
...
- no git client by default.  This seems a huge omission, maybe I need to install lazygit before I see something. Otherwise I will check the LazyVim Extras for Neogit)
- completion uses ++return++ key and arrow keys to navigate (rather than ++Tab++)
- no key mapping to save, e.g. ++SPC+"w"++, so using `:w` for now (maybe there is a save on quit - no, but it is possible to save the current session, although I would need to test if that also includes unsaved changes). It seems quiting LazyVim does prompt to save unsaved files (as least one of them).
...
Eric: I appreciate generic sub-domains more, especially as those sub-domains are pealled away and mastered then it makes the system easier to reason about.
